...
export function useSimulacaoChat(simulacaoId: string) {
  ...
  const loadMessages = async () => {
    try {
      const { data, error } = await supabase
        .from('mensagens_chat_simulacoes')
        .select('*')
  ...
  const loadDocuments = async () => {
    try {
      const { data, error } = await supabase
        .from('documentos_chat_simulacoes')
        .select('*')
        .eq('simulacao_id', simulacaoId)
        .order('created_at', { ascending: true })
  ...
  useEffect(() => {
    const addInitialMessage = async () => {
      if (messages.length === 0 && !loading) {
        await sendMessage(
          'Chat iniciado para esta simulação. Pode enviar mensagens e anexar documentos PDF.',
          'system'
        )
      }
    }
  ...
  const sendSystemResponse = async () => {
    const responses = [
      'Mensagem recebida. Como posso ajudar com esta simulação?',
      'Obrigado pela sua mensagem. A nossa equipa irá analisar e responder em breve.',
      'Recebi a sua solicitação sobre a simulação. Vamos proceder com a análise.'
    ]
  ...
  const uploadDocument = async (file: File) => {
    try {
      ...
      const fileName = `simulacoes/${simulacaoId}/${Date.now()}-${file.name}`
